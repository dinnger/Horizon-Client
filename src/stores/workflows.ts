...
export const useWorkflowsStore = defineStore('workflows', () => {
	...
	const loadDefaultWorkflows = async () => {
		const parsed: Workflow[] = await socketService.getWorkflows(projectId.value)
		if (parsed) {
			try {
	...
	const updateWorkflow = async (workflowId: string, data: { nodes: { [key: string]: INodeCanvas }; connections: INodeConnections[] }) => {
